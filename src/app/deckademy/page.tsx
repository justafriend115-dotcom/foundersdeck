...
export default async function AcademyPage() {
  ...
  return (
    <main>
      ...
      <section className="bg-background text-foreground">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-6 py-16 md:grid-cols-2 md:py-20">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3.5">
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-secondary px-6 py-3 text-sm font-bold text-white shadow-lg shadow-background/40 transition-colors hover:bg-secondary"
              >
          ...
          <div className="hidden justify-center md:flex">
            <div className="relative w-full max-w-sm rotate-2 rounded-2xl border border-muted/30 bg-card p-7 text-foreground shadow-[0_32px_80px_rgba(0,0,0,0.35)]">
              <div className="absolute -right-4 -top-4 rounded-xl bg-background px-3 py-2 text-[11px] font-extrabold tracking-wide text-slate-300 shadow-lg">
                {PASS_SCORE}% TO PASS
              </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={s.label} className="rounded-lg bg-card px-2 py-2.5 text-center">
      ...
      <section className="bg-background text-white">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-y-8 px-6 py-9 md:grid-cols-4">
          {[
            ...
            >
              <div className="text-4xl font-black text-slate-400">{stat.num}</div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <Link
              key={link.label}
              href={link.href}
              className="group flex items-center gap-3.5 rounded-2xl border border-secondary bg-card p-5 shadow-[0_4px_16px_rgba(0,0,0,0.07)] transition-all hover:-translate-y-0.5 hover:border-slate-400/40 hover:shadow-[0_10px_28px_rgba(37,99,235,0.14)]"
            >
              <span
                className={cn(
              ...
              >
                <link.icon className="size-5 text-slate-400" />
              </span>
              <span>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="text-center">
            ...
            <h2 className="mt-2 text-3xl font-black tracking-tight text-foreground">
              ...
              <span className="text-slate-400">Ready to Raise</span>
            </h2>
          </div>
            ...
            {ACADEMY_TRACKS.map((track) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "group flex flex-col rounded-2xl border border-secondary bg-card p-5 shadow-[0_4px_16px_rgba(0,0,0,0.05)] transition-all",
                    locked
                      ? "opacity-70"
                      : "hover:-translate-y-1 hover:border-slate-400/50 hover:shadow-[0_12px_32px_rgba(37,99,235,0.12)]",
                  )}
                >
                  <div className="flex items-center justify-between">
                    <span className="flex size-11 items-center justify-center rounded-xl bg-[#1A233A]">
                  ...
                  <div className="mt-4 flex items-center justify-between text-[11px] font-semibold text-slate-400">
                    ...
                    {entry && !locked && (
                      <span className={cn("font-extrabold", certified ? "text-emerald-600" : "text-slate-400")}>
                        {entry.score}%
                      </span>
                    )}
                  </div>
      ...
      {/* NOTICES (progress) */}
      <section className="mt-16 bg-background py-14">
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <Link
              ...
              className="rounded-lg border border-secondary px-4 py-2 text-xs font-bold text-foreground transition-colors hover:border-slate-400/50"
            >
              Continue learning →
            </Link>
          </div>
          ...
          <div className="mt-8 space-y-3">
            {ACADEMY_TRACKS.map((track) => {
              ...
              if (!entry && track.free) {
                return (
                  <div key={track.id} className="rounded-xl border-l-4 border-slate-400 border-y border-r border-secondary bg-card px-5 py-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded bg-secondary px-2 py-0.5 text-[10px] font-extrabold text-slate-400">
                        START HERE
                      </span>
              ...
              if (entry && !entry.passed) {
                return (
                  <div key={track.id} className="rounded-xl border-l-4 border-muted border-y border-r border-secondary bg-card px-5 py-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded bg-primary/10 px-2 py-0.5 text-[10px] font-extrabold text-primary">
                        IN PROGRESS
                      </span>
                      <span className="text-xs font-extrabold text-primary">{entry.score}%</span>
              ...
              if (entry?.passed) {
                return (
                  <div key={track.id} className="rounded-xl border-l-4 border-emerald-500 border-y border-r border-secondary bg-card px-5 py-4">
                    <div className="flex flex-wrap items-center gap-2">
      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-6xl">
          <div className="text-center">
            ...
            <h2 className="mt-2 text-3xl font-black tracking-tight text-foreground">
              Your Path to <span className="text-slate-400">Founder Elite</span>
            </h2>
          </div>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    unlocked
                      ...
                      : isNext
                        ? "border-slate-400/50 shadow-[0_8px_32px_rgba(94,131,174,0.25)]"
                        : "border-secondary",
                  )}
                >
                  {unlocked && (
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2">
                    <span
                      className={cn(
                        "text-2xl font-black",
                        unlocked ? "text-emerald-600" : isNext ? "text-slate-400" : "text-slate-300",
                      )}
                    >
                      {m.count}
                    </span>
      ...
      <p className="no-print bg-background py-8 text-center text-xs text-slate-400">
